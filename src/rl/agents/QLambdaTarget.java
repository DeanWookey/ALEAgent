 ...
 * @author Craig
 */
public class QLambdaTarget extends RLAgent{
    
    Random random;
    double[][] traces;

    public QLambdaTarget(int numActions, int numFeatures) {
    ...
    public QLambdaTarget(int numActions, int numFeatures, Basis[] functionApproximators) {
    ...
    public int agent_step(double reward, State s) {
        ...
        addSample(lastState,lastAction,reward,s);
        lastState = s;
        lastAction = action;
        return action;
    }
    ...
    public void addSample(State currState, int move, double reward, State newState) {
