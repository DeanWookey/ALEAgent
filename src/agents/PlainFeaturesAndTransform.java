...
import rl.memory.StateManagerCombination;

/**
 *
 * @author itchallengeuser
 */
public class PlainFeaturesAndTransform extends TransformAgent {
    ...
    public PlainFeaturesAndTransform(boolean gui, String game, String pipeBasename, int imageWidth, int imageHeight) {
        super(gui, game, pipeBasename, imageWidth, imageHeight, 4);
        Basis[] functionApproximators = new Basis[numActions];

        System.err.println("No transform agent.");
        int numTerms = 0;
    ...
    protected StateManager getStateManager() {
        return new StateManagerCombination(this.getNumFramesPerState(), getImageWidth(), getImageHeight(), new Transformation2D(getImageWidth(), getImageHeight(), getTransformBasis()));
    }
    ...
    protected BasisFunction[] getTransformBasis() {
        return new BSplineBasis(1, imageHeight, imageWidth, 2, -1, 10, false).getBasisFunctions();
    }
