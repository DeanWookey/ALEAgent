 ...
 */
public class CombinationAgent extends TransformAgent {
    ...
    public CombinationAgent(boolean gui, String game, String pipeBasename, int imageWidth, int imageHeight) {
        super(gui, game, pipeBasename, imageWidth, imageHeight, 4);
        Basis[] functionApproximators = new Basis[numActions];

        System.err.println("Fourier transform agent.");
        int numTerms = 0;
    ...
    protected BasisFunction[] getTransformBasis() {
        BasisFunction[] basis1 = new FullFourierBasis(1, imageWidth, imageHeight, 10).getBasisFunctions();
        BasisFunction[] basis2 = new BSplineBasis(1, imageHeight, imageWidth, 2, -1, 10, false).getBasisFunctions();
        BasisFunction[] concat = new BasisFunction[basis1.length + basis2.length];
        System.arraycopy(basis1, 0, concat, 0, basis1.length);
        System.arraycopy(basis2, 0, concat, basis1.length, basis2.length);
        return concat;
    }
